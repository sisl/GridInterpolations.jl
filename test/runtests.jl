...
function simplexMagic(NDISC::Int=20, NPOINTS::Int=3, checkFileName::AbstractString="", eps::Float64=1e-10)
    ...
    val = transpose([[8.0, 1, 6] [3, 5, 7] [4, 9, 2]]) # transposed magic(3) from matlab

    sGrid = SimplexGrid(1.0:NPOINTS, 1.0:NPOINTS)
...
function checkCounters()
    dims = 2
    nPts = 100
    grid = RectangleGrid(1.0:10, 1.0:10)
    if length(grid) != nPts || dimensions(grid) != dims
        return false
    end
    data = rand(10, 10)
    temp = interpolate(grid, data, [-1.5, 1.5])
    temp = interpolate(grid, data, [1.0, 1.0])
    grid = SimplexGrid(1.0:10, 1.0:10)
    if length(grid) != nPts || dimensions(grid) != dims
        return false
    end
    temp = interpolate(grid, data, [-1.5, 1.5])
...
function testMask()
    ...
    vals = collect(1.0:9)
    mask = falses(9)
    grid = RectangleGrid(1.0:3, 1.0:3)
...
function test_rect_implemented()
    ...
    @test @allocated(interpolants(rgrid, [1, 1])) == 0
    @test interpolants(rgrid, [2, 5]) == ([3], [1.0])
    @test @allocated(interpolants(rgrid, [2, 5])) == 0
    ...
    @test @allocated(interpolate(rgrid, [1, 2, 3, 4], [1, 1])) == 0
    @test maskedInterpolate(rgrid, [1, 2, 3, 4], [1, 1], BitArray([false, false, false, false])) == 1.0
    @test @allocated(maskedInterpolate(rgrid, [1, 2, 3, 4], [1, 1], BitArray([false, false, false, false]))) == 0
...
function test_simplex_implemented()
    ...
    @test @allocated(interpolants(grid, [1, 1])) == 0
    ...
    @test @allocated(interpolate(grid, [1, 2, 3, 4], [1, 1])) == 0
    @test maskedInterpolate(grid, [1, 2, 3, 4], [1, 1], BitArray([false, false, false, false])) == 1.0
    @test @allocated(maskedInterpolate(grid, [1, 2, 3, 4], [1, 1], BitArray([false, false, false, false]))) == 0
...
@testset "Iteration and Indexing" begin
    @testset "2d" begin
        ...
        pts = ([1.0, 4.0], [2.0, 4.0], [3.0, 4.0], [1.0, 5.0], [2.0, 5.0], [3.0, 5.0])
    ...
    @testset "3d" begin
        ...
        pts = reshape([[x, y, z] for x = 1.0:3.0, y = 4.0:5.0, z = 6.0:10.0], length(g3))
